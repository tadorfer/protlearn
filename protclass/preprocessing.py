...
def txt_to_df(X, label):
    ...
    X = pd.read_csv(X, names=['Sequence'], header=None)
    X['Label'] = np.ones(len(X))*label
...
def integer_encode(X, padding=False):
    ...
    label: ndarray of shape (n_samples,)
        Contains the corresponding class labels of the peptide sequences.
    ...
    return enc_arr, np.array(X['Label'])